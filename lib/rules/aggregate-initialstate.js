...
const meta = {
  docs: {
    description: `aggregate-initialstate enforces that 'initialState' is an object.`,
    category: 'wolkenkit',
    recommended: Boolean(recommended['wolkenkit/aggregate-initialstate'])
  },
  schema: []
};
...
const create = function (context) {
  ...
  return {
    ...
    VariableDeclaration (node) {
      const initialStateDeclarator = ast.declarations.findByName(node.declarations, 'initialState');

      if (initialStateDeclarator) {
        state.initialState = {
          declarator: initialStateDeclarator,
          name: initialStateDeclarator.id.name,
          node
        };
      }
    },
