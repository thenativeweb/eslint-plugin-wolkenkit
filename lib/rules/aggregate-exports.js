...
const meta = {
  docs: {
    description: `aggregate-exports enforces that aggregates export 'initialState', 'commands', and 'events'`,
    category: 'wolkenkit',
    recommended: Boolean(recommended['wolkenkit/aggregate-exports'])
  },
  schema: []
};

const defaultMessage = `Expected module to export an aggregate.`;
...
const create = function (context) {
  ...
  return {
    ExpressionStatement (node) {
      ...
      const missingPropertyNames = aggregate.propertyNames.filter(propertyName => !propertyNames.includes(propertyName)),
            surplusProperties = properties.filter(property => !aggregate.propertyNames.includes(property.key.name));

      missingPropertyNames.forEach(property => {
        context.report({
          node,
          message: `Expected aggregate to contain '${property}'.`,
          loc
        });
      });
